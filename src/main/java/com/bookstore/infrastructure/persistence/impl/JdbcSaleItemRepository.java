...
public class JdbcSaleItemRepository extends AbstractJdbcRepository<SaleItem, UUID> {
  ...
  private static final String SQL_SELECT_BY_ID = SQL_BASE_SELECT + "WHERE si.sale_item_id = ?";

  private static final String SQL_SELECT_ALL = SQL_BASE_SELECT + "ORDER BY si.sale_item_id";
  ...
  protected SaleItem mapRow(ResultSet rs) throws SQLException {
    ...
    Sale sale = Sale.builder().id(UUID.fromString(rs.getString("sale_id"))).build();
