...
public class SimpleConnectionPool implements AutoCloseable {
  ...
  /**
   * Атомарний лічильник без synchronized. Відстежує загальну кількість відкритих з'єднань (вільних
   * + зайнятих).
   */
  ...
  public SimpleConnectionPool(PoolConfig config) {
    ...
    System.out.printf("[Pool] Ініціалізовано: %d з'єднань готові%n", config.minConnections());
  }
      ...
      if (conn == null) {
        ...
        if (conn == null) {
          throw new DatabaseException(
              String.format(
                  "З'єднання недоступне впродовж %d мс. Пул вичерпано" + " (%d/%d)",
                  config.timeoutMs(), totalConnections.get(), config.maxConnections()));
        }
      }
    }
  ...
  public void printStats() {
    ...
    System.out.printf(
        "[Pool] Вільних: %d | Зайнятих: %d | Всього: %d/%d | Завантаженість:" + " %.1f%%%n",
