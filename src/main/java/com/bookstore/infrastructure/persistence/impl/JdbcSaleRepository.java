 ...
 *
 * <p>Позиції покупки ({@code items}) завантажуються через {@link LazyList} — відкладено при
 * першому зверненні до {@code sale.getItems()}.
 */
public class JdbcSaleRepository extends AbstractJdbcRepository<Sale, UUID> {
    ...
    private static final String SQL_INSERT =
          """
          INSERT INTO sales
              (sale_id, user_id, client_id, sale_date, total_amount, payment_method)
          VALUES (?, ?, ?, ?, ?, ?)
          """;
    ...
    private static final String SQL_UPDATE =
          """
          UPDATE sales
          SET user_id = ?, client_id = ?, sale_date = ?,
     ...
     * @param saleItemRepository репозиторій позицій для Lazy Loading
     */
    public JdbcSaleRepository(
          ConnectionManager connectionManager, JdbcSaleItemRepository saleItemRepository) {
        super(connectionManager);
        this.saleItemRepository = saleItemRepository;
    }

    /**
     * Data Mapper: ResultSet → Sale з вкладеними User, Client та LazyList позицій.
     *
     * <p>Client може бути null (анонімна покупка) — LEFT JOIN + перевірка client_id. Позиції
     * завантажуються лише при першому виклику {@code sale.getItems()}.
     */
    @Override
    protected Sale mapRow(ResultSet rs) throws SQLException {
        ...
        return Sale.builder()
              ...
              // LazyList: позиції завантажуються лише при sale.getItems()
              .items(new LazyList<>(() -> saleItemRepository.findBySaleId(saleId)))
              .build();
    }
    ...
    public List<Sale> findByClientId(UUID clientId) {
        ...
        try (Connection conn = connectionManager.getConnection();
            ...
            throw new DatabaseException("Помилка findByClientId: " + clientId, e);
        }
        return list;
    }

    /**
     * Знаходить всі покупки оформлені касиром.
     *
    ...
    public List<Sale> findByUserId(UUID userId) {
        ...
        try (Connection conn = connectionManager.getConnection();
            ...
            throw new DatabaseException("Помилка findByUserId: " + userId, e);
        }
        return list;
    }
}