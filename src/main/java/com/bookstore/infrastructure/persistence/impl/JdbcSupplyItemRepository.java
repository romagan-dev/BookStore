...
public class JdbcSupplyItemRepository extends AbstractJdbcRepository<SupplyItem, UUID> {
  ...
  private static final String SQL_SELECT_BY_ID = SQL_BASE_SELECT + "WHERE si.supply_item_id = ?";

  private static final String SQL_SELECT_ALL = SQL_BASE_SELECT + "ORDER BY si.supply_item_id";
  ...
  protected SupplyItem mapRow(ResultSet rs) throws SQLException {
    ...
    Supply supply = Supply.builder().id(UUID.fromString(rs.getString("supply_id"))).build();
